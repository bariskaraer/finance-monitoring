...
export class ChargingManager<ChargeEventId extends string> {
    ...
    public static async initialize<ChargeEventId extends string>(): Promise<ChargingManager<ChargeEventId>> {
        ...
        const runInfo = await Actor.apifyClient.run(Actor.getEnv().actorRunId!).get();
        if (!runInfo.pricingInfo || 
            !('apifyMarginPercentage' in runInfo.pricingInfo) ||
            !('createdAt' in runInfo.pricingInfo) ||
            !('startedAt' in runInfo.pricingInfo)) {
            throw new Error('Invalid pricing info from API - missing required properties');
        }
        
        // Convert string dates to Date objects if needed
        if (typeof runInfo.pricingInfo.createdAt === 'string') {
            runInfo.pricingInfo.createdAt = new Date(runInfo.pricingInfo.createdAt);
        }
        if (typeof runInfo.pricingInfo.startedAt === 'string') {
            runInfo.pricingInfo.startedAt = new Date(runInfo.pricingInfo.startedAt);
        }

        const typedRunInfo = runInfo as unknown as ActorRunCorrectType<ChargeEventId>;

        const chargeState = {} as ChargeState<ChargeEventId>;

        if (runInfo.chargedEventCounts && runInfo.pricingInfo.pricingModel === 'PAY_PER_EVENT' && runInfo.pricingInfo.pricingPerEvent.actorChargeEvents) {
